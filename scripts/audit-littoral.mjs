...
mkdirSync("audits/littoral", { recursive: true });
...
for (const [vw,vh,dpr,label] of [[1440,900,1,"desktop"],[390,844,2,"mobile"]]) {
  ...
  for (const p of [["home-fr","/"],["home-en","/en"],["resa-fr","/reservation"],["resa-en","/en/reservation"]]) {
    ...
    await sweep(page);
    await page.screenshot({path:`audits/littoral/${p[0]}-${label}.png`,fullPage:true});
...
for (const p of [0,0.5,1.0]) {
  ...
  await page.screenshot({path:`audits/littoral/hero-${Math.round(p*100)}.png`});
...
for (const [n,i] of [["verriere",1],["suite",3],["piscine",6]]) {
  ...
  await page.screenshot({path:`audits/littoral/espaces-${n}.png`});
...
await page.screenshot({path:`audits/littoral/galerie-traversee.png`});
...
await page.screenshot({path:`audits/littoral/lightbox.png`});
console.log("✓ lightbox");
await page.keyboard.press("Escape");
await page.waitForTimeout(400);
await page.locator("button[aria-label*='Ouvrir la conversation']").click();
await page.waitForTimeout(800);
await page.screenshot({path:`audits/littoral/widget-mael.png`});
...
await rp.screenshot({path:"audits/littoral/calendrier-selection.png"});
console.log("✓ calendrier-selection");
await rp.locator("button",{hasText:"Continuer"}).click();
await rp.waitForTimeout(500);
await rp.locator("button",{hasText:"Continuer"}).click();
await rp.waitForTimeout(700);
await rp.screenshot({path:"audits/littoral/etape3-recap.png"});
...
await mp.screenshot({path:"audits/littoral/nav-mobile-ouverte.png"});
...
await fp.screenshot({path:"audits/littoral/footer.png"});
console.log("✓ footer");
await fp.close();

await browser.close();
console.log("Audit → audits/littoral/");
